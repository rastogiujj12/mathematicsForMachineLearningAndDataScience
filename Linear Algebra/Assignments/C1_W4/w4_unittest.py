...
def test_cov_matrix(target_cov_mat):
    successful_cases = 0
    failed_cases = []
    # file 'expected_cov_mat.npy' split into 8 files because file size exceeded github limit of 100mb
    npy = np.load('expected_cov_mat_0.npy')
    for index in range(1, 8):
        npy = np.vstack((npy,np.load(f'expected_cov_mat_{index}.npy')))
